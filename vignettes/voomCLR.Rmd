...
```{r}
patient <- rep(paste0("patient", 1:(n/2)), 2)
v <- voomCLR(
  counts = Y,
  design = design,
  lib.size = NULL
...
# Empirical Bayes estimation of the bias

In exceptional cases, it can be useful to adopt an empirical Bayes approach to estimating the bias term. Such exception cases include experiments where

 - Cell type abundance change is measured as a function of distance to an entity, these distances are then binned and the differential abundance analysis is performed for every bin;
 - A longitudinal setting where many subjects are followed over several timepoints, and one expects the compositional bias to be similar across timepoints.

These situations share that the compositional bias can be expected to be similar across several distance bins or time points. In such a scenario it is natural to assume that the bias terms across all distance bins or time points come from the same distribution.
